...
export function createVibeMcpServer(): McpServer {
  ...
  server.registerTool(
    "list_vibe_styles",
    {
      ...
      description:
        "List available UI styles and motion interaction specs from the Vibe library.",
      inputSchema: {
        type: z
          .enum(["ui", "motion", "all"])
          .optional()
  ...
  server.registerTool(
    "search_vibe_styles",
    {
      ...
      description:
        "Search UI styles and motion specs by keyword, domain, aesthetic, or type.",
      inputSchema: {
        query: z
          .string()
          .optional()
          .describe(
            "Keyword to match id, name, description, triggers, domains, or aesthetics.",
          ),
        type: z
          .enum(["ui", "motion", "all"])
          .optional()
  ...
  server.registerTool(
    "get_vibe_style",
    {
      ...
      description:
        "Fetch the full markdown specification for a UI style or motion interaction by id, name, or trigger keyword.",
      inputSchema: {
        id: z
          .string()
          .describe(
            "Style id, English name, Chinese name, or trigger keyword (e.g. stripe, fluid-elastic-button-hover).",
          ),
        type: z
          .enum(["ui", "motion"])
          .optional()
  ...
  server.registerTool(
    "get_vibe_combo",
    {
      ...
      description:
        "Fetch one UI style and one or more motion specs together — ideal for building a complete page vibe.",
      inputSchema: {
        ui: z
          .string()
        ...
        motions: z
          ...
          .min(1)
          .describe(
            "Motion spec ids, names, or triggers (e.g. fluid-elastic-button-hover).",
          ),
      },
    },
    async ({ ui, motions }) => {
      ...
      const motionItems = motions.map((motionId) =>
        findItem(catalog, motionId, "motion"),
      );
  ...
  server.registerResource(
    "style",
    new ResourceTemplate("vibe://style/{type}/{id}", {
      list: async () => {
        const catalog = await loadCatalog();
        return {
    ...
    async (uri, variables) => {
      ...
      const item = findItem(
        catalog,
        idValue,
        typeValue as CatalogItemType | undefined,
      );
  ...
  server.registerPrompt(
    "build-with-vibe",
    {
      ...
      description:
        "Prompt template for building a page with a specific UI style and motion interactions.",
      argsSchema: {
